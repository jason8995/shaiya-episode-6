...
void Configuration::LoadChaoticSquareData()
{
    try
    {
        ...
        for (int num = 1; num < recipeMax + 1; ++num)
        {
            ...
            auto vec = ext::split<std::vector>(result, L',', 3);
            if (vec.size() != 3)
                continue;
            ...
            for (int i = 0; i < 24; ++i)
            {
                ...
                auto vec = ext::split<std::vector>(value, L',', 3);
                vec.resize(3, L"0");
        ...
        for (int num = 1; num < squareMax + 1; ++num)
        {
            ...
            auto value = ini.GetValueOrDefault(key, L"");
            auto vec = ext::split<std::vector>(value, L',', 2);
            if (vec.size() != 2)
                continue;
            ...
            vec = ext::split<std::vector>(value, L',');
            if (vec.empty())
                continue;
